...
public class RevokeMsgHook {
    ...
    public void hook(final XC_LoadPackage.LoadPackageParam loadPackageParam) {
        ...
        try {
            // insert method
            XposedBridge.hookAllMethods(HookClass.storageFromRevokeMsgclass, HookClass.revokeMsMethodname,
                      new XC_MethodHook() {
                          @Override
                          protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                              if (!PreferencesUtils.getRecallMsg()) {
    ...
    private void handleMessageRecall(ContentValues contentValues) {
        ...
        XposedHelpers.callMethod(storageInsertClazz, HookClass.revokeMsMethodname, msg, false);
    }
}
